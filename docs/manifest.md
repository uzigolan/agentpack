...
| `portablePack` | no | Self-contained runtime/config payload copied into target artifacts; use `agentpack pack import DIRECTORY` to create this entry. Imports record `importedFrom`, and `build`/`package` refresh the copied payload from that source before writing artifacts. |
