...
def build(
    ...
    if embed_token:
        ...
        if not supported.intersection(selected):
            typer.secho(
                "--embed-token requires the copilot or codex target.", fg=typer.colors.RED
            )
            raise typer.Exit(code=2)
        token = typer.prompt("Bearer token (token only)", hide_input=True).strip()
        if not token:
            typer.secho("Bearer token cannot be empty.", fg=typer.colors.RED)
            raise typer.Exit(code=2)
...
def mcp_import_cmd(
    ...
    # HTTP MCPB bridges are generated by AgentPack. Drop the legacy producer
    # bundle list when an HTTP definition is refreshed.
    if any(server_doc.get("transport", {}).get("type") == "http" for _, server_doc in servers):
        doc.pop("claudeDesktopMcpb", None)
